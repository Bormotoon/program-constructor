...
Собранная версия — `npm run build`, результат в `dist/`. Сборка не требует
сервера: `base` относительный, поэтому её можно положить в подкаталог сайта,
раздать со статического хостинга или открыть `index.html` прямо с диска.
Готовый архив каждой версии лежит в
...
- **вес загрузки** — стартовый бандл 89 КБ в gzip. Всё тяжёлое вынесено в
  отдельные чанки и грузится по нажатию: DOCX (103 КБ), PDF со встроенным
  шрифтом (207 КБ), разбор Excel (117 КБ); тематические планы предметов —
  по выбору предмета. Выгрузка в текст и Markdown весит 1 КБ: ей ничего,
  кроме самих данных, не нужно;
